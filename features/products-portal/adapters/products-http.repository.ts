...
export class ProductsHttpRepository implements ProductsRepository {
  async searchProducts(params: ProductsSearchParams): Promise<PaginatedProducts> {
    ...
    return mapRawProductsResponse(raw, { page: params.page, pageSize: params.pageSize })
  }
  ...
  async getCategories(): Promise<string[]> {
    ...
    return parsed.map((item) => (typeof item === "string" ? item : item.slug ?? item.name ?? ""))
  }
  ...
  async updateProduct(id: ProductId, input: UpdateProductInput): Promise<Product> {
